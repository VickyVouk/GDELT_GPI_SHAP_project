...
For example, for Yemen, the prediction for June 2018 is 3.23, which corresponds to the 1-month-ahead prediction.
...
<sup>Figure 2: It presents the model output value, i.e., the GPI estimation of June 2018, and the base value, which is the value that would be predicted if the variables for the current output were unavailable. The plot also displays the most important variables that the model uses for the GPI estimation, such as ``Discuss by telephone'' and ``Provide military aid''. The red arrows are the variables that push the GPI estimation higher, and the blue ones push the estimation lower.</sup>
